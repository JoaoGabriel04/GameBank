...
import { getPropData, groupByColor, sortPropItems, sortSessionPosses } from "@/utils/properties";
...
export default function Inicio({ onNavigate }: InicioProps) {
  ...
  const completedColorGroups = useMemo(() => {
    ...
    return propertyGroups.filter((g) => {
      // Grupo completo E não é grupo de ações — ações não têm casas/hotéis,
      // o rendimento delas é fixo por dados, então não permitem construção.
  ...
  return (
    <div className="space-y-6 px-4 sm:px-6 lg:px-10">
      ...
      <Modal
        ...
        onClose={() => { setModalNegociar(false); resetNegociarCreate(); }}
      >
