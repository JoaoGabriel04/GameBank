...
type Props = {
  variant?: "floating" | "tab";
  onUnreadCount?: (n: number) => void;
};

export default function Chat({ variant = "floating", onUnreadCount }: Props) {
  ...
  }, [variant, messages.length, onUnreadCount]);
  ...
  useEffect(() => {
    if (variant === "floating" && !open && messages.length > 0) {
      setUnreadCount((prev) => prev + 1);
    }
  }, [messages.length, open, variant]);
  ...
  const handleSend = () => {
    ...
    if (trimmed.length > MAX_CARACTERES) return;
    const lines = trimmed.split("\n").length;
    if (lines > MAX_LINHAS) return;
  ...
  const enrichText = texto;
  const charCount = enrichText.length;
  const lineCount = enrichText.split("\n").length;
  ...
  const chatContent = (
    <>
          ...
          >
            <div className="px-4 py-3">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <p className="font-inconsolata text-[10px] text-red-400 uppercase tracking-wider">Votação de expulsão</p>
                  <p className="font-inconsolata text-xs text-zinc-300 mt-0.5">
                    <span className="text-zinc-100 font-semibold">{kickVote.initiatorNome}</span> propôs expulsar{" "}
              ...
              {isEligible && !alreadyVoted && (
                <div className="flex gap-2">
                  <button onClick={() => handleKickVote("no")} className="flex-1 py-1.5 bg-zinc-800 hover:bg-zinc-700 border border-zinc-600 text-zinc-300 rounded font-inconsolata text-[11px] transition-colors cursor-pointer">Não</button>
                  <button onClick={() => handleKickVote("yes")} className="flex-1 py-1.5 bg-red-900/50 hover:bg-red-800/70 border border-red-700/50 text-red-300 rounded font-inconsolata text-[11px] font-semibold transition-colors cursor-pointer">Sim, expulsar</button>
                </div>
              )}
              {!isEligible && !alreadyVoted && (
      ...
      <div className="px-4 py-3 border-t border-zinc-700 flex flex-col gap-2 shrink-0">
        <div className="flex gap-2">
          <textarea
            value={texto}
            ...
            rows={1}
            className="flex-1 bg-zinc-800 text-zinc-100 text-sm rounded-lg px-3 py-2 border border-zinc-600 focus:outline-none focus:border-green-500 font-inconsolata resize-none"
          />
          <button
            onClick={handleSend}
            disabled={!texto.trim() || isCooldown}
        ...
        <div className="flex items-center justify-between text-[10px] font-inconsolata px-1">
          <span className={`${texto.length > MAX_CARACTERES ? "text-red-400" : "text-zinc-500"}`}>
            {charCount}/{MAX_CARACTERES}
          </span>
          {isCooldown && (
            <span className="text-amber-400">Aguarde...</span>
          )}
        </div>
      </div>
    </>
  ...
  return (
    <>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[1.25rem] h-5 bg-green-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1 border-2 border-zinc-900">
