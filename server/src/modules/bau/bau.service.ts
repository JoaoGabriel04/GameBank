...
export class BauService {
  ...
  async concederBauPartidaAdmin(userId: number, tipo: TipoBau) {
    const bau = await bauRepository.findBauByTipo(tipo)
    if (!bau) throw new AppError(500, "Baú não encontrado no banco")
    const { BAU_TEMPO_ESCOLTA } = await import("../../constants/baus.js")
    const timerMin = BAU_TEMPO_ESCOLTA[tipo] ?? 10
    const unlockAt = new Date(Date.now() + timerMin * 60_000)
    return bauRepository.createBauAdquirido({
      userId, bauId: bau.id, sessionId: undefined, gameResultId: undefined, position: 0, unlockAt,
    })
  }
  ...
  async abrirMultiplo(userId: number, tipo: TipoBau, quantidade: number) {
    const config = BAU_CONFIG[tipo]
    if (!config) throw new AppError(400, "Tipo de baú inválido")

    if (!Number.isInteger(quantidade) || quantidade < 2 || quantidade > 50) {
      throw new AppError(400, "Quantidade deve ser entre 2 e 50")
    }

    const totalCoins = config.precoCoins ? config.precoCoins * quantidade : 0
    const totalDiamonds = config.precoDiamonds ? config.precoDiamonds * quantidade : 0
    ...
    return withLock(`bau:${userId}`, async () => {
      ...
      if (!user) throw new AppError(404, "Usuário não encontrado")
      if (totalCoins && user.coins < totalCoins) {
        throw new AppError(400, `Coins insuficientes. Necessário: ${totalCoins.toLocaleString("pt-BR")}`)
      }
      if (totalDiamonds && user.diamonds < totalDiamonds) {
        throw new AppError(400, `Diamantes insuficientes. Necessário: ${totalDiamonds.toLocaleString("pt-BR")}`)
      }

      return this._executarMultiplo(userId, tipo, quantidade, totalCoins, totalDiamonds)
    })
  }

  private async _executarMultiplo(
    userId: number, tipo: TipoBau, quantidade: number,
    totalCoins: number, totalDiamonds: number
  ) {
    const config = BAU_CONFIG[tipo]
    const bau = await bauRepository.findBauByTipo(tipo)
    if (!bau) throw new AppError(500, "Baú não encontrado no banco")
    ...
    const batches: {
      ...
      itens: {
        id: number; name: string; type: string; value: string | null
        imageUrl: string | null; raridade: Raridade; animated: boolean
        fragmentosGanhos: number; fragmentosAtuais: number
        fragmentosTotal: number | null; itemCompleto: boolean
      }[]
    }[] = []
    ...
    for (let i = 0; i < quantidade; i++) {
      ...
      const pegarItem = async (raridadeAlvo: Raridade): Promise<ItemSorteado | null> => {
        const deltas = [0]
        for (let step = 1; step < 5; step++) { deltas.push(step); deltas.push(-step) }
        const idx = ORDEM_RARIDADES.indexOf(raridadeAlvo)
        for (const delta of deltas) {
          const rIdx = idx + delta
          if (rIdx < 0 || rIdx >= ORDEM_RARIDADES.length) continue
      ...
      itensSorteados.sort((a, b) => ORDEM_RARIDADES.indexOf(a.raridade) - ORDEM_RARIDADES.indexOf(b.raridade))

      const temFragmentosPendentes = itensSorteados.length > 0
      let xpBonus = 0
      const xpConfig = XP_BONUS[tipo]
      if (!temFragmentosPendentes && xpConfig) xpBonus = randInt(xpConfig.min, xpConfig.max)
      ...
      // Transação curta por baú — sem risco de timeout
      await prisma.$transaction(async (tx) => {
        await tx.user.update({ where: { id: userId }, data: { coins: { increment: coinsGanhos } } })

        if (xpBonus > 0) {
          const userAtual = await tx.user.findUnique({ where: { id: userId }, select: { xp: true, level: true } })
          if (userAtual) {
            const { xp, level } = addXp(userAtual.xp, userAtual.level, xpBonus)
        ...
        for (const item of comXp) {
          ...
          const totalNecessario = item.fragmentosTotal ?? 999
          if (fragmentoAtual && fragmentoAtual.quantidade >= totalNecessario && !ownedItemIds.has(item.id)) {
            const userAtual = await tx.user.findUnique({ where: { id: userId }, select: { user_items: true } })
            const refs = parseUserItems(userAtual?.user_items)
            refs.push({ item_id: item.id, equipped: false, acquiredAt: new Date().toISOString() })
            await tx.user.update({ where: { id: userId }, data: { user_items: refs as any } })
            await tx.userFragment.delete({ where: { userId_itemId: { userId, itemId: item.id } } })
            itensCompletos.push(item.id)
          }
        }
      })
      ...
      batches.push({
        ...
        xpBonus: xpBonus > 0 ? xpBonus : undefined,
        itens: comXp.map(item => ({
          id: item.id, name: item.name, type: item.type, value: item.value,
          imageUrl: item.imageUrl, raridade: item.raridade, animated: item.animated,
          fragmentosGanhos: distribuicao.get(item.id) ?? 1,
          fragmentosAtuais: fragMap.get(item.id) ?? 0,
          fragmentosTotal: item.fragmentosTotal,
          itemCompleto: itensCompletos.includes(item.id),
