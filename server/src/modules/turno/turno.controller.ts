...
export const turnoController = {
  ...
  usarCartaPrisao: async (req: Request, res: Response) => {
    try {
      ...
      const result = await turnoService.usarCartaPrisao(sessionId, player.id);
      res.status(200).json(result);
    } catch (err) {
      parseError(res, err);
    }
  ...
  revelarDados: async (req: Request, res: Response) => {
    try {
      const sessionId = z.coerce.number().int().positive().parse(req.params.sessionId);
      const userId = req.user!.userId;
      ...
      const result = await turnoService.revelarDados(sessionId, player.id);
      res.status(200).json(result);
    } catch (err) {
      parseError(res, err);
    }
